...
class MNG1000Controller extends AppController
{
    ...
    public function index()
    {
        ...
        $this->Session->write('MNG1000:users', $users);
    ...
    public function add()
    {
        ...
        $this->Session->write('MNG1000:' . $token, $emptyUser);
    ...
    public function postEdit()
    {
        ...
        $this->Session->write('MNG1000:' . $token, $targetUser);
        $this->redirect(['controller' => 'MNG1000', 'action' => 'edit', '?' => ['t' => $token]]);
    }
    ...
    public function getEdit()
    {
        ...
        $targetUser = $this->Session->read('MNG1000:' . $token);
    ...
    {
        $token = $this->request->data('txtToken');
        if (is_null($token)) {
            throw new BadRequestException('トークンが未指定です。');
        }

        if (!$this->Session->check('MNG1000:' . $token)) {
            $this->setAlertMessage('対象ユーザの編集は完了しています。', 'notice');
            $this->redirect(['action' => 'index']);
            return;
        }
