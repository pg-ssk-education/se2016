...
class CMN1000Controller extends AppController {
	...
	public function index() {
		...
		$this->set('invalidAccessCount',
			$this->InvalidAccess->findCountByClientIpOnLastOneMinute($this->request->clientIp(false)));
	}
	...
	public function login() {
		...
		if (empty($user)) {
			$this->InvalidAccess->saveClientIp($this->request->clientIp(false));
		...
		$this->InvalidAccess->deleteAll(
			['InvalidAccess.CLIENT_IP' => $this->request->clientIp(false)], false);
